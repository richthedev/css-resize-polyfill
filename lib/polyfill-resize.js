...
function DragResize(myName, config)
{
 var props = {
  ...
  vertical: true,                  // Allow horizontal resize.
...
DragResize.prototype.mouseMove = function(e) { with (this)
{
 ...
 else
 {
  ...
  if (horizontal) elmX += diffX;
  if (vertical) elmY += diffY;
 }
...
(function() {
  ...
  var resizeHandlerPolyfill = function(target, options) {
    if (!target) {
    ...
    if (target.length) {
      ...
      for (var i = target.length; i--;) {
        resizeHandlerPolyfill(target[i], options);
      }
    } else {
      ...
      if (resizeSupported && !options.force) {
        return;
      }
